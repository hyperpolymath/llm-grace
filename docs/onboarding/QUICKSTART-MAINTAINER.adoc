...
// Replace rsr-template-repo, llm-grace, {{DEPS}} with actuals
...
| `$PREFIX/share/llm-grace/`
| Data files, assets

| `$PREFIX/share/doc/llm-grace/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/llm-grace/config.toml`

Fallback: `$HOME/.config/llm-grace/config.toml`
...
Or via OPSM: `opsm update llm-grace`
...
[source,bash]
----
just install --prefix=/opt/llm-grace-instance1 --config=/etc/llm-grace/instance1.toml
just install --prefix=/opt/llm-grace-instance2 --config=/etc/llm-grace/instance2.toml
----
