...
# LLM_GRACE ABI/FFI Documentation
...
│       └── include/
│           └── llm_grace.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── llm_grace.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/llm_grace.h
```
...
int main() {
    void* handle = llm_grace_init();
    if (!handle) return 1;

    int result = llm_grace_process(handle, 42);
    if (result != 0) {
        const char* err = llm_grace_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    llm_grace_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lllm_grace -L./zig-out/lib
```

### From Idris2

```idris
import LLM_GRACE.ABI.Foreign
...
#[link(name = "llm_grace")]
extern "C" {
    fn llm_grace_init() -> *mut std::ffi::c_void;
    fn llm_grace_free(handle: *mut std::ffi::c_void);
    fn llm_grace_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = llm_grace_init();
        assert!(!handle.is_null());

        let result = llm_grace_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libllm_grace = "libllm_grace"

function init()
    handle = ccall((:llm_grace_init, libllm_grace), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:llm_grace_process, libllm_grace), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:llm_grace_free, libllm_grace), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/llm_grace.h
   ```
