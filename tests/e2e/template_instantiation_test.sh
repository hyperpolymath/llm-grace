...
find "$TEST_REPO_PATH" -type f \
    ...
    -exec bash -c '
        file="$1"
        placeholder_pairs=(
            "llm-grace|$TEST_REPO_NAME"
            "hyperpolymath|$TEST_OWNER"
            "github.com|$TEST_FORGE"
            "LLM_GRACE|$TEST_PROJECT_NAME"
            "llm_grace|'"${TEST_REPO_NAME//-/_}"'"
            "{{REPO_DESCRIPTION}}|$TEST_DESCRIPTION"
            "Zig|$TEST_PRIMARY_LANGUAGE"
            "Jonathan Jewell|$TEST_AUTHOR"
            "j.d.a.jewell@open.ac.uk|$TEST_AUTHOR_EMAIL"
            "2026-05-18|2026-04-04"
        )
